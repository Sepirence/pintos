...
/* Starts a new thread running a user program loaded from
 ...
 process_execute (const char *file_name)
 {
   ...
   struct thread* child =list_entry(list_rbegin(&thread_current()->childList), struct thread, elem);
...
is_parent_of(pid_t pid)
{
  ...
  for(elem = list_bigin(&thread_current()->childList); elem!= list_end(&thread_current()->childList); elem = list_next(elem))
  {
...
static struct cell* get_fd(int fd)
{
  struct list_elem *elem;
  struct cell *newFd = NULL;
  struct list *fdList = &thread_current()->fdList;

  for(elem=list_begin(fdList);elem!=list_end(fdList);elem=list_next(elem))
  {
...
process_exit (void)
{
  ...
  int size = list_size(&openfile_list);
   for(; size > 0; size--)
   {
      ...
      of = list_entry(e, struct cell, elem);
...
process_open(const char *file_name)
{
  ...
  if(f==NULL)
    return -1;
  struct cell *newFd = malloc(sizeof(struct cell));
  if(newFd == NULL)
    return -1;
  newFd->fd = allocate_fd();
  newFd->file = f;
  list_push_back(&thread_current()->fdList,&newFd->elem);
  return newFd->fd;
}
...
process_read(int fd, void *buffer, unsigned size)
{
  struct cell *newFd;
  if((newFd=get_fd(fd))!=NULL)
    return file_read(newFd->file,buffer,size);
  else
    return -1;
}

int
process_write(int fd, void *buffer, unsigned size)
{
  struct cell *newFd;
  if(fd == STDOUT_FILENO)
  {
  ...
  else if((newFd= get_fd(fd))!=NULL)
    return file_write(newFd->file,buffer,size);
  else
    return -1;
}
...
process_seek(int fd, unsigned position)
{
  struct cell *newFd;
  if((newFd = get_fd(fd))!=NULL)
    return file_seek(newFd->file, position);
  else
    return -1;
}
